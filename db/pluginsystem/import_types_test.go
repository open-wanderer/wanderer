...
func TestTrailImportDifficultyRejectsNonStringJSON(t *testing.T) {
	for _, payload := range []string{`{"difficulty":1}`, `{"difficulty":true}`, `{"difficulty":[]}`, `{"difficulty":{}}`} {
		var item TrailImport
		if err := json.Unmarshal([]byte(payload), &item); err == nil {
			t.Errorf("expected decoding error for %s", payload)
		}
	}
}
