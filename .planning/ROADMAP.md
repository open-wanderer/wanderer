...
**Success Criteria** (what must be TRUE):
  ...
  5. An admin toggles a region on and its boundary is cached in `region_geometry`; an admin hovers a disabled region, sees its true boundary outline, and no row is written.
