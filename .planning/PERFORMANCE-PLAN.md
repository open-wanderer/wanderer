...
| P3 | 1.7 geolocator background/route-visibility gating + 1.8 misc (tracelet relaunch reconciliation, `active_nav.read` limit 1, stats ticker stop while frozen) | GPS duty cycle down outside recording | Medium (lifecycle edge cases) |
| P4 | 2.1–2.5 pan-path memoization | Chip at the remaining ~64% pan CPU (Dart share) | Low-medium |
| P5 | 0.3–0.7 network hygiene (images, glyphs, retry, health, proxy) | Cold-start and flaky-network radio use | Low |
