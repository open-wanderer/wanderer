...
**Status: ALL PHASES COMPLETE AND DEVICE-VERIFIED (2026-08-08).** Original framing below; Findings from a 4-way parallel codebase audit
...
| P6 | 3.1–3.3 isolates + memory — **DONE, VERIFIED ON DEVICE 2026-08-08.** Details: `toModel({includeGpx})` — the library and profile own-trails lists pass `false`, so opening them no longer XML-parses every downloaded trail on the UI thread NOR retains parsed `Gpx` + raw XML per row (detail screens re-read full models); trail-detail open parses server GPX via `compute(parseGpxSafely, xml)`; file import parses and re-serializes via compute; recording-Stop/planner-Finish run `trailFromGpx` metrics + `GpxWriter` serialization via compute (`trailFromGpxIsolate`/`serializeGpxToXml` top-level entry points in conversion.dart — compute with explicit messages, NEVER capturing closures: an `Isolate.run(() => ...)` closure captured the enclosing scope's BuildContext/binding and threw unsendable). Planner undo/redo stack capped at 20 snapshots. **Deliberately skipped:** trimming `trailSearchProvider`'s append-only pages (breaks pagination bookkeeping + scrollback for a session-bounded cost) and `mergeRouteIntoTrail`'s sync serialize (planner-edit path, sync signature). | Library jank gone; peak PSS well under 1 GB | Medium (async refactor) |
| P7 | 3.4–3.5 cleanups + leftover 2.7 items — **DONE, VERIFIED ON DEVICE 2026-08-08.** Details: topography painter repaints throttled 60/120→~30 Hz via a republished clock notifier (ticker infra already mutes it under opaque routes/app pause); toast auto-dismiss uses cancellable Timers; `DateFormat` memoized per locale for list items (`dateFormatYMMMMd`/`dateFormatYMMMd` in format.dart); library search debounced 250 ms; `computeTrailMetrics` memoized per Gpx via a weak `Expando` (TrailPanel); trail-detail AppBar fade moved to a scoped `ValueNotifier` (was a whole-screen setState per scrolled pixel) and its retired-row redirect guarded against per-frame rescheduling; the two map-sheet result lists converted to `ListView.builder` (index 0 header + keyed `TrailCard` rows, empty state extracted); keys added to card builders on library/list/profile screens; `MapOptions` cached (keyed on `disabled`) in TrailMap + TrailCollectionMap, completing the P4 fix across all hosts. **Skipped as not-worth-it:** `existsSync` in card builds (one µs-level stat; a static memo risks stale thumbnails), global/location search `ListView(children:)` (result sets are single-page-bounded), scroll-opacity notifiers on list/profile screens (no metrics recompute behind them), elevation-scrub ValueNotifier refactor (map hosts now cache MapOptions, removing the JNI cost that made it matter). | Minor | Low |
