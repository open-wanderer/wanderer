...
1. **Exact display-`name` derivation rule for leaf/group rows**
   ...
   - RESOLVED: strip the exact parent `comaps_id` + `_` prefix from the compound id (a depth-0 node's name is its full `comaps_id`). This is safer than a naive "text after the last `_`" split against place names that themselves contain underscores. Implemented in plan 28-01 Task 2's name-derivation behavior.
...
2. **Whether to also fetch/cross-check `countries.txt`'s explicit leaf/group marker (see A2)**
   ...
   - RESOLVED: rejected — hierarchy.txt indentation is sufficient and already verified against real fixtures; countries.txt cross-check adds fetch surface for marginal benefit. hierarchy.txt's indentation is a single authoritative source already byte-inspected against real CoMaps data this session (HIGH confidence on "CoMaps format" per Metadata), and adding a second fetched file as a cross-check increases the network-dependency surface of a maintainer-run tool for marginal benefit. If a leaf-looking node with no real `.poly` ever exists, plan 28-02's `.poly` fetch already surfaces it as a loud `log.Fatalf` naming the region — the same fail-loud outcome a countries.txt cross-check would give, without the extra fetch. Plans 28-01/28-02 stay hierarchy.txt-only. (Supersedes the earlier "fetch countries.txt too" recommendation here and A2's "keep countries.txt as a cross-check" note in the Assumptions Log.)
...
3. **`--commit=<hash>` default value (D-02)**
   ...
   - RESOLVED: the default (`defaultCommitHash` in `db/commands/seed_regions.go`) is a concrete 40-hex CoMaps `main` HEAD SHA, resolved at implementation time by plan 28-02 Task 1 (via Codeberg's Forgejo commits API `/repos/comaps/comaps/commits?limit=1&sha=main`, or `git ls-remote https://codeberg.org/comaps/comaps.git main`) and baked into source as a literal string — never the literal `main`, which would defeat D-02's "reproducible without re-editing source" intent. The plan fixes the mechanism and the constraint (concrete literal SHA, not `main`); the exact SHA is captured at implementation time.
