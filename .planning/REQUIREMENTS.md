...
- [ ] **HANDOFF-01**: User can finish planning and hand off the route as a draft Trail (synthesized GPX + named waypoints, with elevation populated via a one-time `/api/v1/valhalla/height` fetch at handoff time — regardless of whether the Elevation tab was ever opened during planning) to the existing trail create/edit screen
