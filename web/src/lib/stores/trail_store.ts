...
async function createTrailWithAssets(trail: Trail, photos: File[], gpx: File | Blob | null, f: (url: RequestInfo | URL, config?: RequestInit) => Promise<Response>, user: AuthRecord | undefined, onOmitted: AssetImportOmissionHandler) {
    ...
    const formData = objectToFormData({ ...trail, public: false }, ["photos", "expand", "_assetLinks", "_assetPluginLinks", "routing_provenance"])
    if (trail.routing_provenance !== undefined) {
        formData.set("routing_provenance", JSON.stringify(trail.routing_provenance));
    }
...
async function updateTrailWithAssets(oldTrail: Trail, newTrail: Trail, photos: File[] | undefined, gpx: File | Blob | null | undefined, exclude: (keyof Trail)[] | undefined, onOmitted: AssetImportOmissionHandler) {
    ...
    const formData = objectToFormData(newTrail, ["expand", "photos", "_assetLinks", "_assetPluginLinks", "routing_provenance", ...(exclude ?? [])])
    if (newTrail.routing_provenance !== undefined && !exclude?.includes("routing_provenance")) {
        formData.set("routing_provenance", JSON.stringify(newTrail.routing_provenance));
    }
