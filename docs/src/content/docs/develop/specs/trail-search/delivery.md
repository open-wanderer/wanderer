...
1. **[SRCH0 abgenommen](/develop/specs/trail-search/work-items/search/srch0/)
   ...
   Vollständigkeit oberhalb des bestehenden Cluster-Caps sind davon
...
Am 19. September 2026 wurde `SRCH0-GAP-SORT-001` als kleine
Robustheitsverbesserung zurückgestellt: Die normale Oberfläche erzeugt
gültige Sortierwerte; ein Fehler im gewöhnlichen Gebrauch ist für den
negativen Storagefall bisher nicht nachgewiesen. Die Bereinigung ungültiger
Storagewerte und die Fallback-Richtung sind keine SRCH0-Abnahmevoraussetzung;
vorerst ist dafür kein eigener PR vorgesehen. Die Diagnosefälle bleiben
erhalten. Die vorhandene Suite bildet diese nicht blockierende Einordnung
...
Ebenfalls **kein SRCH0-Blocker** ist die globale Tag-Umbenennung
`SRCH0-MUTATION-008`: Sie ist nur administrativ möglich; die normale UI
bietet keine Umbenennung, die Records-API sperrt sie für normale Benutzer.
Der isolierte Fix bleibt auf `fix/search-tag-metadata` (`122974380`) lokal
ab `dev` (`c73966d6c`) vorbereitet, ohne Push, PR oder Integration.
Die [Tag-Einstufung](/develop/specs/trail-search/work-items/search/srch0/#nicht-blockierende-tag-umbenennung)
grenzt diesen Diagnosefall von verbindlichen Tagfiltern und Tagzuordnungen
ab; andere Metadatenbefunde sind nicht pauschal ausgenommen. Die Suite muss
die Trennung von Diagnose und Abnahme noch nachführen.

Auch negative Thumbnailindizes `SRCH0-PROJECTION-021` sind **kein
SRCH0-Blocker**. Der Absturz der Suchprojektion ist mit einem tatsächlich
gespeicherten Datensatz und Foto nachgewiesen; die normale Fotoauswahl
erzeugt solche Werte jedoch nicht, und die JSON-API weist sie zurück.
Der separate lokale Fix auf `fix/search-thumbnail-index` fällt bei
negativen Indizes auf das erste Foto zurück, ohne API-Validierung oder
Datenbankschema zu ändern. Die [Vorschaubild-Einstufung](/develop/specs/trail-search/work-items/search/srch0/#nicht-blockierende-vorschaubild-absicherung)
hält den begrenzten Umfang und die noch nachzuführende Diagnose-/Abnahmetrennung
...
keine Produktänderung, kein neues UI-Signal, keine Änderung von `maxTotalHits`.
...
bewahren Diagnose und historische Evidenz. Tests und Korpus bleiben
unverändert; die technische Trennung von Diagnose und Abnahme steht noch
aus. Zugriffsscope, Sichtbarkeit und Berechtigungen bleiben verbindlich.
Push, PR und Integration des Importfixes sind noch nicht erfolgt.
...
| ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------------------ |
| <span id="baustein-srch0"></span>[SRCH0 – Regressionsbasis](/develop/specs/trail-search/work-items/search/srch0/) | Datierter, ausführbarer Testkorpus mit stabilen Case-IDs, unveränderter historischer Evidenz, korrekten aktiven Erwartungen und unabhängigen Eigenschaften | Implementierung: keine; Abnahme: bekannte Fehler im verbindlichen Abnahmeumfang korrigiert, integrierte Zielrevision grün; die oben abgegrenzten Befunde zu Sortier-Robustheit, Feldauswahl, Tag-Umbenennung, negativen Thumbnailindizes, SDK-HTTP-Statusweitergabe, Actor-Suchparametern, Upload-Duplikatprüfung und Cluster-Cap sind keine Blocker | fachlich korrekte Basis für spätere Änderungen; aktuell `blocked` |
