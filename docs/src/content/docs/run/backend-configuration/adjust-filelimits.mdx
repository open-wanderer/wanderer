...
wanderer brings several mechanisms which allow limiting the maximum file size of content provided by users.
Pictures in the users profile, trail highlights and gpx tracks fall under this file size limit.

<Aside> 
As wanderer runs parts of the logic in the users browser those limits can only be enforced, when trying to save a ressource.
