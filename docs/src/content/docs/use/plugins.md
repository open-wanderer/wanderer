---
title: Plugins
description: How to use installed provider plugins with wanderer.
---

Plugins add optional functionality that is not built into the core application.
Official Wanderer images do not include provider plugin bundles. Once an
administrator has installed one, it appears in the plugin settings page. Most
plugins can then be configured and enabled by each user; host-selected
first-party plugins can have a different default-activation policy.
...
| Routing | Calculate routes, elevation, or turn-by-turn maneuvers. |
...
Valhalla is currently the only default-active first-party plugin. After its
separate release bundle is installed, Wanderer enables it for existing users
who do not already have a Valhalla instance and for users created later. An
existing disabled instance remains disabled. When no engine was explicitly
selected, Wanderer chooses route, elevation, and maneuver providers from the
enabled plugins' capabilities, so no manual engine selection is required when
Valhalla is the only suitable plugin. Disabling Valhalla is the persistent way
to opt out; a missing instance is automatically repaired.
