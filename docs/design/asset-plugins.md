...
Defaults are a two-minute export timeout, a 15-minute session timeout, a 30-second slot-acquisition timeout, and `max(8, runtime.NumCPU())` workers. The minimum covers the routing host's global eight-lane call budget. `WANDERER_PLUGIN_WORKER_*` environment variables configure these runtime boundaries.
