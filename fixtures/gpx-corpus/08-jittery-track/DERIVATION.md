...
CONV-05: the reported distance must be the smoothed accumulator (only advancing on a hop that
clears the 5 m threshold), not the raw sum of every consecutive-pair haversine distance — GPS
jitter that never actually moves the device forward should not inflate the reported distance.
...
asserts the smoothed value (`distance` in `metrics`/`trail`), because that is what
`GPX.getTotals()` reports as `distance` (`gpx.ts:148`, `totalDistance = metrics.totalDistanceSmoothed`).
The raw sum is not part of the public contract at all — D-04 explicitly excludes
`cumulativeDistance` (whose last entry would equal this raw sum) from the corpus, since its only
